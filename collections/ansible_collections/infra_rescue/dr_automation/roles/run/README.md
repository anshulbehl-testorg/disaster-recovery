Infra_rescue.Dr_automation Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: infra_rescue.dr_automation.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from infra_rescue.dr_automation
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: infra_rescue.dr_automation.run
      vars:
        run_x: 42
```
